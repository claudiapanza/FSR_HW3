close all
clear
clc

%% Parameters

load("ws_homework_3_2025.mat");
m = 1.5;
g = 9.81;
I_b = diag([1.2416 1.2416 2*1.2416]);
T_s = 0.001; %s
t = attitude.time;

c_0 = 10;
k_0 = c_0;
r_vec = [1 5 10 20]; % Example vector of r values

%% Loop over r values

colors = lines(length(r_vec)); % For plotting

external_wrench_all = zeros(6, length(t), length(r_vec)); % Store results

for idx = 1:length(r_vec)
    r = r_vec(idx);

    % Transfer function
    s = tf('s');
    G = (k_0/(s+c_0))^r;
    c = cell2mat(G.Denominator);

    % K computation
    K = zeros(r,1);
    Product = 1;
    for i = 1:r
        K(i) = c(i+1)/Product;
        Product = Product*K(i);
    end
    K = flip(K);

    % Estimator variables
    external_wrench = zeros(6, length(t));
    gamma = zeros(6, length(t), r);
    e_3 = [0 0 1]';
    p_dot_b = linear_vel.signals.values;
    eta_b = attitude.signals.values;
    eta_dot_b = attitude_vel.signals.values;
    thrust_data = thrust.signals.values;
    tau_data = tau.signals.values;

    for k = 1:length(t)-1
        C = compute_C(eta_b(k,:), eta_dot_b(k,:), I_b);
        Q = compute_Q(eta_b(k,:));
        R_b = compute_R_b(eta_b(k,:));
        M = compute_M(eta_b(k,:), I_b);

        q(:,k+1) = [m*eye(3) zeros(3,3); zeros(3,3) M]*[p_dot_b(k+1,:) eta_dot_b(k+1,:)]';

        for i = 1:r
            if i == 1
                gamma(:,k+1,1) = gamma(:,k,1) + K(1)*( (q(:,k+1) - q(:,k)) - T_s*(external_wrench(:,k) + [m*g*e_3 - thrust_data(k)*R_b*e_3; C'*eta_dot_b(k,:)' + Q'*tau_data(k,:)'] ));
            else
                gamma(:,k+1,i) = gamma(:,k,i) + K(i)*T_s*(-external_wrench(:,k) + gamma(:,k,i-1));
            end
        end
        external_wrench(:,k+1) = gamma(:,k+1,r);
    end
    external_wrench_all(:,:,idx) = external_wrench;
end

%% Plots
%% Plot only \hat{f_x}

figure()
hold on
for idx = 1:length(r_vec)
    plot(t, squeeze(external_wrench_all(1,:,idx)), ...
        'LineWidth', 1.5, 'Color', colors(idx,:));
end
plot(t, ones(length(t),1), '--k', 'LineWidth', 1.2) % Disturbance reference
hold off

xlabel('t (s)', 'Interpreter', 'latex')
ylabel('$\hat{f_x}$ (N)', 'Interpreter', 'latex')
legend_entries = arrayfun(@(r) sprintf('$r = %d$', r), r_vec, 'UniformOutput', false);
legend([legend_entries {'Disturbance'}], 'Interpreter', 'latex', 'Location', 'southeast')
grid on
title('$\hat{f_x}$ vs Time for Different $r$', 'Interpreter', 'latex')
%% Real mass of the UAV
% Parametri noti

d_z_hat = -2.5;               % Disturbo stimato lungo z [N]

% Calcolo della massa reale
m_real = m + d_z_hat / g;

% Stampa del risultato
fprintf('La massa reale stimata è: %.4f kg\n', m_real);

